...
class ProfileView: UIView {
    ...
    private let tokenField: UITextField = {
        ...
        field.text = "19c48aff0dae4a20b5dd2eb322ae37a2"
    ...
    func setupLayout() {
        ...
        tokenField.text = SecretStore.shared.get(forKey: "token")
    ...
    @objc func fetchCurrentUser() {
        ...
        let service = UserService.init(session: URLSession.shared, path: "/users", token: token)

        service.show(slug: "current") { [weak self] response in
            guard let that = self else { print("Oups! Could not find reference to self"); return }
            guard let resp = response else { return }
            if resp.status == RespnseStatus.success {
                print("[Succesful]")
